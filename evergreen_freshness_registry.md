...
| For Brokers (`rxbs.org/for-brokers`) | Wix + Service/BreadcrumbList JSON-LD | 2026-07-14 (launched) | Conversion page: refresh = proof-band anchors when 2026 metrics land, service-card accuracy; Useful-with-clients block updated Jul 22-23, 2026: `/standards` + `/what-we-are-seeing` links ADDED, CLL link re-pointed to the rxbs.org canonical; `/results` joins when it goes live (money-page linking rule) |

## Not live yet (enter the pool at launch — add a row with launch date)

the 2 remaining guides (choose-an-auditor, audit-vs-broker-review) · `/how-we-charge` · `/results`
