...
2b. **Revealed demand.** If `funnel_intelligence.md` exists, read its top "Demand signals for the build" section (maintained monthly by `/funnel-intel` from the toolkit-lead download patterns) and let strong signals weight topic choice — a topic buyers are actively downloading toolkits about outranks an editorially-equivalent alternative. Performance data still wins where they conflict.
...
## Step 4c — Monthly Q&A thread (only when the week contains a month's LAST Thursday)
If the week being built contains the last Thursday of a month, include the **Monthly Q&A Thread** (subscriber-only, 7:30 AM that Thursday): a PART 2B section drafted from `substack_qa_template.md` (theme matched to the season/pillar arc), the Reference Channel "Monthly Q&A" header image prompt (canonical prompt in `newsletters/week_17_carve_out_decision.md`, only the Plex Mono month label changes), and its Run of Show row. Number consecutively from the prior thread (W30 = #4, W34 = #5). Weeks without a last Thursday skip this step silently.
...
Apply the deep-dive-visual rule: a Tuesday/Thursday visual repurposed into the Monday deep dive goes INTO PART 1 as `### In-Article Visual`, not the backlog, not a dead DO-NOT-SHIP block. Genuinely-parked drafts (paused format / displaced topic) go to `evergreen_visual_backlog.md` or `field_note_backlog.md` with full spec. Zero REPURPOSED/DO-NOT-SHIP carcasses remain. **Every `### In-Article Visual` MUST carry a ready-to-paste `#### AI Image Generation Prompt` (ChatGPT/Gemini), not just an "Infographic Concept" layout description — the concept can't be generated as-is.** Write the prompt to the Substack image-prompt recipe + clean-scorecard/conceptual AI-generation guidance (brand palette, traffic-light coding where scored, IBM Plex Sans SemiBold title, thumbnail-legible, no logos in the prompt; add the PBS logo in Canva after).

**Visual-composition dedup (required, added Jul 2, 2026):** before finalizing this week's Tue + Thu visuals, scan the last ~4 weeks' Tuesday/Thursday visual **shapes** — central metaphor/object + composition + dominant palette motif ("dollar routed across a map," "marked-up contract page," "traffic-light scorecard," "balance scale") — not just topics. The topic look-back in Step 1 does NOT catch visual-metaphor repeats (this is how W27 Thursday duplicated W26 Thursday). If this week's planned visual is the same shape as one within ~4 weeks, change the metaphor/composition or the format class before writing the prompt. Record the recent-visual-shapes list next to the recent-topics list in the build notes.
