...
Drafted Substack Field Notes that have been displaced from their original week and are available to slot into a future Thursday. Parallels `evergreen_visual_backlog.md` (visual concepts) and `repurpose_queue.md` (viral-hit redistribution).

**Required content-building behavior:**

(a) When filling a Thursday Field Note slot — especially for weeks where the planned topic has shifted, the deep dive's natural Thursday companion is unclear, or a holiday week needs a low-effort fill — open this file first and check whether a backlog draft fits the slot's pillar before drafting from scratch.

(b) When a backlog draft is used, lift the body, meta, and header image prompt from this file, refresh dates and any time-sensitive references to the publishing week, update the **Pillar:** marker if the destination week's pillar mix has changed, and update the inventory table's Status column from `available` to `published [date, week-N]`.

(c) Time-sensitive references (e.g., "this week," "April 21," CMS announcements) must be re-anchored or removed before publishing — backlog drafts go stale on the news cycle that produced them.
...
## FN-01: One Drug Class to Watch: The Next Wave of GLP-1 Approvals

**Original week:** 18 (May 7, 2026 publish date — needs re-anchoring before reuse)
**Pillar:** Clinical Pharmacy
**Length:** ~550 words (~3 min read)
**Tone:** Practical, tactical
**Why it was displaced:** W18 Thursday Field Note was retargeted to Channel Pricing so the Thursday LinkedIn visual (3-bottle messy infographic) and the Plan Sponsor Toolkit handout (Channel Pricing Audit Worksheet PDF) ship together as a unified Thursday package. The GLP-1 field note remains a strong standalone Clinical Pharmacy field note and should be slotted into a future Thursday whose Monday topic does not already cover GLP-1 in depth.

**Re-anchoring notes before reuse:**
- "Monday's article covered pipeline monitoring broadly" — re-anchor to that week's actual Monday topic, or rewrite the opening if Monday's topic is unrelated.
- Confirm the indication-expansion list (cardiovascular, NASH/MASH, sleep apnea, chronic kidney disease) is still current; add or remove indications based on FDA approvals between displacement date and re-publish date.
- Confirm oral-formulation and biosimilar references reflect the state of the market at re-publish.

---

### Field Note Body (FN-01 draft)
...
### Field Note Header Image Prompt (FN-01 draft)
...
## FN-03: How Plan Sponsors Actually Enforce Audit Rights

**Original week:** 23 (June 11, 2026 publish date — needs re-anchoring before reuse)
**Pillar:** PBM Contract Insights
**Length:** ~580 words (~3 min read)
**Tone:** Practical, tactical
**Why it was displaced:** W23 Field Note was retargeted to Admin Fees per the May 8 two-track architecture. Tuesday Library 03 already covers Audit Rights and routes to the Contract Language Library; making the Thursday Field Note ALSO about audit rights would over-saturate the week on a single theme. Field Note retargeted to pair with the Thursday Slot A "5 Admin Fee Terms Decoded" LinkedIn carousel instead. This Audit Rights Field Note remains a strong standalone PBM Contract Insights field note and should be slotted into a future Thursday whose Tuesday LinkedIn is NOT about audit rights — natural fits include any week paired with a Library NN entry on a non-audit provision family (Renewal/Amendment, Specialty Routing, Formulary Override, etc.).
...
### Field Note Header Image Prompt (FN-03 draft)
