...
- **July 7, 2026 — Mark Cuban FOLLOWED @ginny_crisp (relationship/distribution MILESTONE) + liked the "definition of a word in your contract" aphorism (P1).** The FOLLOW is the material event, distinct from a repost: it is a standing distribution change (Ginny's posts now surface directly in Cuban's feed), which structurally raises the repost probability on every future post rather than lifting one. The like is logged as a like only (per the May 8 likes-only rule, a Cuban like alone drives zero reach); its value here is a targeting signal confirming the "definition/word in the contract" shape (P1) is the one that catches his eye. Context: landed at the tail of a heavy engagement day (~20 substantive replies across the PBM/transparency ecosystem, several to Cuban's own clean-lane posts on opacity, the confidentiality clause, and the conglomerate/"no receipt" point). **Operational takeaway:** the channel's amplifier is now a follower; keep the strongest self-dealing / definition / offshore-aggregator Cuban-bait in steady rotation, and when he engages, ship a same-vein follow-on into the live-attention window. Discipline note reaffirmed same day: engage Cuban's *clean PBM/transparency* posts fast; SKIP his political/M4A threads (no clean lane, pulls positioning sideways); don't stack near-duplicate replies on back-to-back posts.
