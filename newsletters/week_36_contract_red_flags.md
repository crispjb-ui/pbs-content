# Week 36 Newsletter Bundle: Contract Red Flags and Fixes
## September 7-11, 2026
...
| Wed Sep 9 | 8:00 AM | X | recipe / Implementation allowance clawback (Tier B) | PART 4B · Wed | [ ] |
...
**Publish:** Monday, September 7, 2026 at 7:30 AM EST
...
**The contract language comparison library**

Ten provisions. Three versions of each: weak (favors the PBM), moderate (some protection), and strong (protects the plan). Each includes a plain-language explanation of what the language means in practice.
...
> A clean, flat corporate infographic, 1080 x 1350 pixels (4:5 vertical), on a white (#FFFFFF) background with generous margins, designed as a printable weak-versus-strong contract-language comparison table (flat editorial style, not photoreal, no paper or handwriting texture). Top: a full-width Primary Blue (#015880) header band about 180px tall containing the two-tone title "Contract Language" in white IBM Plex Sans SemiBold (about 46pt, clinical register, never a condensed poster face) with "Weak vs Strong" in Accent Blue (#A7E0FA) IBM Plex Sans SemiBold (about 32pt) directly beneath, left-aligned. Below the header, a thin Gray (#4D4D4D) IBM Plex Sans line "10 provisions. Which version is in your contract?" Body: five evenly spaced comparison rows, each row split into a Red-tinted "WEAK" left cell and a Green-tinted "STRONG" right cell, with the current contract phrase set in IBM Plex Mono. The five rows in order: WEAK "PBM shall provide competitive pricing" (Red #C0392B label "Undefined, unenforceable") next to STRONG "GER of [X]% by channel, quarterly true-up" (Green #2E7D32); WEAK "PBM shall pass through manufacturer rebates" (Amber #F5A623 label "What counts as a rebate?") next to STRONG "All compensation from manufacturers"; WEAK "Plan may audit during normal business hours" (Red label "No access to manufacturer contracts") next to STRONG "Including manufacturer agreements relevant to Plan"; WEAK "Penalties capped at $50,000" (Red label "The cap limits your protection") next to STRONG "Dollar-for-dollar, no cap"; WEAK "PBM may amend with 30-day notice" (Red label "They can change your terms") next to STRONG "Mutual written agreement required". Render every number, including "$50,000" and "30-day", in IBM Plex Mono tabular figures. Column headers "WEAK" in Red (#C0392B) and "STRONG" in Green (#2E7D32), IBM Plex Sans SemiBold. Large, high-contrast, thumbnail-legible type throughout, capped at these five rows so nothing else competes. Reserve empty space at the bottom-right for a logo (do not draw one), and place a small footer line "rxbs.org" in Gray (#4D4D4D) IBM Plex Mono at the bottom-left. No grid-pattern background, no photographs, no people, no real logos, no PBM logos. Color palette strictly: white (#FFFFFF), Primary Blue (#015880), Accent Blue (#A7E0FA), Gray (#4D4D4D), plus Green (#2E7D32), Amber (#F5A623), and Red (#C0392B) for the weak-versus-strong scoring.
...
**SEO URL Slug:** pbm-contract-language-comparison-library
**Tags:** PBM contracts, contract language, negotiation, self-funded employers, pharmacy benefits
...
Your broker or benefits counsel should review these provisions with you before renewal. The best advisory teams maintain their own contract language benchmarks and bring specific language recommendations to the table.

---

This week's Benefit Blind Spots includes a **contract language comparison library**: 10 provisions in weak, moderate, and strong versions with plain-language explanations. Designed as a reference document for renewal. That library is exclusive to the Substack: [SUBSTACK URL]
...
"Commercially reasonable efforts" vs. "best efforts." One gives the PBM an exit. The other does not. I found this in a client's contract on a pricing guarantee they were relying on. The guarantee used "commercially reasonable efforts." When the PBM missed the target, they argued that meeting it was not commercially reasonable given market conditions. The contract supported their position.

The employer had relied on a promise the contract did not actually make.
...
```
Rebate guarantees cover rebates. So PBMs stopped calling the money rebates. "Data fees." "Inflation payments." "Administrative reimbursement." Same manufacturer checks, new labels, and every relabeled dollar exits your pass-through clause. First red flag to check in any contract review: how narrow is the definition of "rebate"?
```
...
```
Your discount guarantee is only as honest as the benchmark under it. AWP, WAC, MAC: the PBM assigned one to each line of the contract, and it maintains at least one of them itself. The benchmark choice was the real negotiation. You were not in the room for it.
```
- **5:00 PM · punchy:**
```
Vague language in a PBM contract is not sloppy drafting. It is inventory. Every undefined term gets monetized before the term ends.
```
...
- **5:00 PM · punchy:**
```
Contract review rule: read every guarantee twice. Once for the promise. Once for the definitions section that takes the promise back.
```
...
**Wed Sep 9**
- **8:00 AM · recipe / Implementation allowance clawback (Tier B; concept swapped Jul 9, 2026: formulary-exclusion material is reserved for Thursday's reveal this week):**
```
That "implementation allowance" the PBM offered at signing was not a gift. It is a loan. The repayment is amortized into your rates, and if you terminate early the unamortized balance comes due in full. The generosity is collateral. Find the clawback schedule your contract never calls a clawback schedule.
```
  First reply (link): rxbs.org/toolkit/termination-clause-audit
- **12:00 PM · news-reaction HOLD** (fire same-hour on breaking PBM news; tag the source). Quiet-day backup:
```
The three biggest PBMs own the pharmacies, the rebate aggregators, and in one case the doctors. Vertical integration is a polite phrase for being every counterparty in your own transaction.
```
- **5:00 PM · punchy:**
```
Ten provisions decide most of your PBM contract's economics. The vendor hopes you spend the entire negotiation on one of them: the discount.
```
...
```
How a PBM rescues a generic guarantee it is missing: reprice a few high-volume generics in December, or deepen the discount on drugs your members never fill. The annual average lands exactly on target. Your monthly invoices never did. The red flag is one word: "annual."
```
  First reply (link): rxbs.org/toolkit/ger-audit
- **12:00 PM · news-reaction HOLD** (fire same-hour on breaking PBM news; tag the source). Quiet-day backup:
```
"We pass through 100% of rebates" and "your plan receives all manufacturer money" are different sentences. Only one of them ever appears in a contract, and it is not the second.
```
...
```
A client's invoice at renewal: the headline admin fee "held flat," and three new fees appeared beneath it. Clinical support. Network management. Data access. The rate held. The invoice grew. Unbundling is how a price increase gets past a procurement review.
```
  First reply (link): rxbs.org/toolkit/pbm-compensation
- **12:00 PM · news-reaction HOLD** (fire same-hour on breaking PBM news; tag the source). Quiet-day backup:
```
...
```
Your oncology clinic used to buy and bill the infusion drug itself. Now the PBM mandates the drug ship from its own specialty pharmacy: white bagging. If the dose changes on infusion day, the shipped vial is waste and the clinic cannot substitute its own stock. Ask what problem the mandate solved, and for whom.
```
...
- **8:30 PM · aphorism [C9]:**
```
Fees named after services are cheaper to invent than services. Your PBM invoice is the proof of concept.
```
