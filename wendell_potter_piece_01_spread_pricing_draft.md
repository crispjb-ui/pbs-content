...
**Status:** Working draft. ~910 words. Piece 1 of the HEALTH CARE un-covered contributorship roster (follows the Opening/Map piece). Drafted June 2026. Fact-checked + bill named (June 2026).
...
2. **Mechanism — REVISED for accuracy + bill named.** The original "FTC final order under Section 5, no new statute required" overstated it: the PBM reports came from the FTC's **6(b) market study** (not enforcement), and a *blanket industry-wide* disclosure mandate via a single Section 5 order is not clean. Revised to the accurate two-track framing: the FTC built the record via the 6(b) study, **opened a Section 5 enforcement case against the three PBMs in 2024** (the GPO/insulin action), and a disclosure requirement is the next step on that track — with the statutory path being the **Pharmacy Benefit Manager Transparency Act of 2025, S.526** (Sen. Grassley R-IA + Sen. Cantwell D-WA, reintroduced Feb 11, 2025). The bill **bans spread pricing and requires PBMs to report spread, fees, clawbacks, and the affiliated-vs-non-affiliated reimbursement difference** — i.e., exactly the disclosure the piece prescribes. Body names the Act + sponsors (cleaner for the genre); "S.526" is here for the editor. Confirm committee/floor status before publication if a status line is wanted (the bill cleared Senate Commerce in the prior Congress; verify the 119th status).
...
Article body word count: ~910 words. Within Wendell's 800-1,000 range.
