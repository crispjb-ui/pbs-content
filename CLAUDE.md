...
- **Branch workflow (added June 12, 2026):** Ginny works on `main`. Before a session ends, merge the session's work into `main` and push (standing permission given June 12, 2026: "push everything to main, that is where i work"). Don't leave finished work stranded on session branches; Ginny's local clone pulls `main` for scheduling and the video pipeline.
...
- **Comment-and-reply value rule (added July 3, 2026, required behavior on every LinkedIn / X / Substack reply and comment PBS writes):** A reply must ADD, never echo. **Do not restate or paraphrase the commenter's point back to them as the substance of the reply** — repeating what they already said, even in fresh words, is filler and reads as AI. A brief acknowledgment token is allowed and encouraged for warmth (name them, a short "agreed," or quote ONE of their phrases as a springboard), but the *body* of every reply must deliver something net-new the comment did not already contain: a different angle, a named structural mechanism, a proprietary PBS anchor, a plan-sponsor-actionable next step, a sharper reframe, a new sourced fact, or a question that moves the thread forward. **Pre-send test (mandatory): "What does this reply add that the comment did not already say?" If the honest answer is "nothing, it just agrees or repeats," rewrite before sending.** Match depth to the comment: a technical/expert comment earns a technical add; a warm/human comment earns a warm, specific reply (warmth counts as value, but it must be specific, not generic praise); an empty vendor plug earns a Like or a single vendor-neutral line, not an invested reply (never amplify content-free pitches). All standing brand rules bind inside replies exactly as in posts: brokers-as-allies / PBM-as-adversary (never co-sign a comment that blames brokers — reframe the fault to the PBM structure), vendor-neutral on named competitors (affirm the principle, not the product), no em dashes or hyphens as sentence separators, one unfakeable proprietary anchor wherever the reply is substantive, no fabricated numbers. This is the reply-side complement to the Humanize Check: that test asks "could only PBS have written this"; this one asks "does this reply move the thread forward." Applies to Claude-drafted replies and to Ginny's own; not a week-file post rule, so it needs no `/propagate-rule` pass.

  **Refinement (added July 4, 2026, from the Adam Fein 340B exchange — sharpens the rule, does not replace it):** two failure modes surfaced that the base rule did not catch, plus the technique that works.
  (1) **Springboard, do not re-explain.** Naming the other person's concept as a launch pad is allowed ("your co-optation point is the whole case"); *re-explaining their concept back to them in your own words is not.* Every reference to their idea must immediately build something new on top of it, never restate it as the body. If a sentence could be deleted and the reader would lose only a paraphrase of what they already said, delete it.
  (2) **Do not restate the POST'S OWN THESIS.** The base rule guarded against parroting a commenter; the harder trap is writing a long "insightful" comment on someone's *post* that is actually that post's central argument handed back at more length. This is acute for expert/authority posts (Fein, Chase, Platt) and for videos: **you must know what the author already argued before claiming to add to it.** If you have not seen the full content (whole video, full post, not just the slides/excerpt), say so and frame contributions as extensions, or ask. Adding at length what the author already said reads as not having engaged.
  (3) **The move that works: acknowledge direction in one clause, then contribute what the author did NOT say** — a genuine qualifier, a productive tension, a different stakeholder, a named mechanism, a forward-looking migration. Prefer *critical extension or respectful disagreement* over agreement-elaboration (take up any "agree or disagree?" invitation as disagreement or complication, since agreement almost always collapses into echo). Canonical example from the Fein thread: he said "as list price declines toward net, the 340B spread shrinks"; the value-add was not "yes it shrinks" but "the compression is uneven (it migrates to the newest high-list specialty drugs) and per-claim spread and total extraction diverge, so the exposure relocates rather than retires." That reply accepts his mechanism and then says something he did not. **Pre-send test v2: "Is my core point already the author's own point?" If yes, it is not a comment, it is a summary — find the qualifier, the tension, or the stakeholder they left out, or do not post."
