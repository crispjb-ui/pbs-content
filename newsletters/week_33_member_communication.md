# Week 33 Newsletter Bundle: Communicating Pharmacy Changes
## August 17-21, 2026
...
**Subtitle:** Five pharmacy change scenarios with complete messaging frameworks: member letters, FAQs, provider outreach, and language to avoid

**Publication:** Benefit Blind Spots
**Publish:** Monday, August 17, 2026 at 7:30 AM EST
...
Five scenarios. For each: the advance notification letter, the FAQ framework, the call center script, the provider communication, and language to use and language to avoid. These are the templates we use with clients.
...
> A clean, flat corporate infographic, 1080 x 1350 pixels (4:5 vertical), on a white (#FFFFFF) background with generous margins, designed as a printable one-page communication checklist (not photoreal, no whiteboard texture). Top: a full-width Primary Blue (#015880) header band about 180px tall containing the two-tone title "The Communication Checklist" in white IBM Plex Sans SemiBold (about 46pt, clinical register, never a condensed poster face) with "That Prevents the Backlash" in Accent Blue (#A7E0FA) IBM Plex Sans SemiBold (about 30pt) directly beneath, left-aligned, and a small white outline megaphone icon at the right of the band. Below the header, a thin Gray (#4D4D4D) IBM Plex Sans line "5 scenarios. Same framework." Body: four evenly spaced numbered step rows, each row led by a rounded Primary Blue pill holding the step number in white IBM Plex Mono tabular figures, then the step label in Primary Blue IBM Plex Sans SemiBold (about 26pt) with its timing and detail in Gray IBM Plex Sans beneath. The four rows in order: pill "1" — "Notify Providers First" — "60 days before. Clinical rationale. Exception process."; pill "2" — "Notify Members" — "45 days before. Name the drug. Explain why. Confirm doctor knows."; pill "3" — "Brief the Call Center" — "30 days before. Drug-specific FAQ. Escalation path."; pill "4" — "Follow-Up Reminder" — "7 to 10 days before. Short. Reassuring. Action steps clear." Render every day count in Plex Mono tabular figures. Below the four rows, a single two-column comparison block titled "Language" in Gray IBM Plex Sans SemiBold: a Green (#2E7D32) left column headed "WORKS" listing "FDA-approved", "your doctor has been informed", "same or lower copay"; a Red (#C0392B) right column headed "BACKFIRES" listing "cost savings initiative", "cheaper alternative", "you must switch"; both lists in IBM Plex Sans, high contrast, generously spaced. Large, thumbnail-legible type throughout, nothing beyond the four steps and the one comparison block competing for attention. Reserve empty space at the bottom-right for a logo (do not draw one), and place a small footer line "rxbs.org" in Gray (#4D4D4D) IBM Plex Mono at the bottom-left. No grid-pattern background, no photographs, no people, no real logos, no PBM logos. Color palette strictly: white (#FFFFFF), Primary Blue (#015880), Accent Blue (#A7E0FA), Gray (#4D4D4D), plus Green (#2E7D32) and Red (#C0392B) for the works-versus-backfires columns.
...
**Tags:** member communication, pharmacy changes, biosimilar transition, formulary changes, self-funded employers
...
## The Pharmacy Benefits Briefing - Issue #28
### Member Communication Briefing

**Publish:** Monday, August 17, 2026 at 7:45 AM EST
**Length:** ~400 words
...
Here is what I have learned: pharmacy benefit changes are uniquely sensitive. Members associate drug changes with quality-of-care changes. "We are switching you to a biosimilar" gets heard as "we are taking away your medication." The interpretation is wrong, but it is real. If the communication does not proactively address the emotional response, the clinical rationale does not matter.

Five scenarios create the most communication friction: formulary tier changes, biosimilar transitions, specialty pharmacy routing changes, prior authorization implementation, and pharmacy network changes. Each requires specific messaging. Generic "your benefits are changing" letters do not work. Specific letters that name the drug, explain the reason, confirm the doctor was informed, and reassure about coverage continuity do work.

The coordination sequence matters: provider communication first (60 days), member communication second (45 days), call center briefing third (30 days), follow-up reminder (7-10 days). This ensures the doctor knows before the member calls.

---

This week's Benefit Blind Spots includes a **communication template library**: five pharmacy change scenarios with advance notification letters, FAQ frameworks, call center scripts, provider communications, and specific language to use and avoid. That library is exclusive to the Substack: [SUBSTACK URL]
...
**Publish:** Tuesday, August 18, 2026 at 7:30 AM EST
...
```
Before you trust any pass-through guarantee, find the sentence that defines what gets passed. The percentage is marketing. The definition is the contract.
```
- **5:00 PM · punchy:**
```
A PBM's real product is a contract you will misread in its favor.
```
...
```
Somewhere in your PBM contract a clock is running toward auto-renewal. Miss the notice window, usually 90 to 180 days out, and every rate you meant to renegotiate rides along for another full term. Put the date on your calendar this week. The PBM already has it on theirs.
```
  First reply (link): rxbs.org/toolkit/termination-clause-audit
- **12:00 PM · news-reaction HOLD** (fire same-hour on breaking PBM news; tag the source). Quiet-day backup:
```
An effective rate guarantee measures the year's average, not your claims. The PBM can miss on every drug your members actually take and still hit the number.
```
- **5:00 PM · punchy:**
```
The quarterly review deck answers every question except the ones that cost the PBM money. That is not an agenda accident.
```
...
```
Your audit right comes with a chaperone. One audit per year, from a preapproved auditor list, on a claims sample the PBM assembles, with findings the PBM may dispute into a drawer. Each restriction sounds reasonable on its own. Stacked, they are a wall.
```
  First reply (link): the PBM Contract Language Library
- **12:00 PM · news-reaction HOLD** (fire same-hour on breaking PBM news; tag the source). Quiet-day backup:
```
PBM member letters are legally accurate and operationally radioactive. "Your medication is being removed" means "moved tiers" in PBM English. The panic lands on your HR team, not theirs. Rewrite the letter before it mails.
```
- **5:00 PM · punchy:**
```
Ask a PBM a hard question in the quarterly review and you get a beautiful slide about a different question.
```
...
```
MAC pricing sounds like a standard. It is a spreadsheet your PBM maintains, edits, and declines to publish. The same generic can carry different MAC prices at different pharmacies on the same day. You signed the methodology. You never saw the sheet.
```
...
- **5:00 PM · punchy:**
```
Every vague term in a PBM contract was left vague on purpose, by the side that drafted it. Precision would cost them money.
```
...
```
Manufacturer money reaches your PBM under many names: rebate, admin fee, data fee, price protection. Your pass-through clause captures exactly one of those words. The other checks clear just fine. They simply never reach your plan.
```
...
```
- **2:30 PM · 340B contract pharmacy spread (Tier A)** _(re-concepted Jul 9: the prior NADAC public-list-vs-private-list body was W35's reserved Thursday-reveal scene running two weeks early)_:
```
...
- [ ] Monday-Friday posts per schedule, daily Notes
