...
- **Hook & retention refresh (required every run):** re-verify and update the bank's **"Hook & retention playbook"** section — current hook structures, pattern-interrupt cadence, re-engagement/open-loop techniques, cold-open teaser usage, benchmarks (3s-hold, completion thresholds). When a technique is renderable, check whether `Clip.tsx` already implements it (midHook, coldOpen, punch-ins, karaoke, animated end card); if a new renderable technique emerges, propose the component addition to OPEN_ITEMS rather than leaving it prose-only. Retire playbook items the sources no longer support. Vet everything for B2B brand fit (Humanize Check: no influencer hype voice).
