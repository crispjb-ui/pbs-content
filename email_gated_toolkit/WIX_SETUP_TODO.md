...
- [ ] Fix the "phantom box" UX issue: form's success behavior currently shows inline message that gets hidden by the cover box. Switch to redirect-to-thank-you-page approach (build a Wix thank-you page using `email_gated_toolkit/landing_pages/thank_you_template.html` as design reference). Once form submits, visitor goes to a new page — no post-submit form state to clean up. **This build also unblocks the paid-ads LinkedIn conversion event (see next item) — the redirect gives a stable `/thank-you` URL to fire the conversion on.**
- [ ] **LinkedIn Ads conversion event (paired with the thank-you page above; queued Jul 6, 2026).** Depends on the thank-you-page redirect existing first. Decision recorded: use a **URL-based conversion**, NOT the event-based/Velo route (chosen for robustness — no fragile edits to the live Velo form code, and it also serves Google Ads/GA4). The LinkedIn Insight Tag is **already live** (Custom Code, all pages, Partner ID `9364474`, added Jul 6, 2026), so only the conversion definition remains. Setup once the `/thank-you` page is live (~5 min, no code): LinkedIn Campaign Manager → **Measurement → Conversion tracking** → **Create conversion** → name `Toolkit Lead` → method **"By URL / page visit"** → match rule **URL contains `/thank-you`** (wildcard-covers every toolkit's `/toolkit/<slug>/thank-you`) → attribution default (each), 30-day click / 7-day view → associate with the ad account. This makes campaigns optimize toward leads, not clicks. Confirm it registers a test submission before the first ad dollar. (Insight Tag status flips Unverified→Active in Signals manager ~24h after live site traffic.)
