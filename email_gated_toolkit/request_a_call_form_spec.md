...
- [x] Ginny: page **fully built and live** at `rxbs.org/request-a-call` (Jul 8, 2026) — form + surrounding copy (eyebrow/H1/subhead/reassurance/secondary-email lines) all in. Notification chain wired via Zapier (Catch Hook → team notification email w/ lead tier/downloads/source → requester confirmation email), **both tested and confirmed**. Topic dropdown "Claims review" → **"Pharmacy Benefits Review"** (Velo `TOPIC_MAP` synced in repo; `?topic=` key = `pharmacy-benefits-review`). _Two small confirms left on this page:_ (a) JSON-LD schema pasted into SEO Settings (§1c); (b) the "Privacy" link stays link-suppressed until the `/privacy` page publishes (counsel-gated).
