...
### Trend log
- **Jul 15, 2026 — SECOND WIX RUN: first non-branded flip, PBS leads Wix's competitor read.** Scores: **ChatGPT 15% (3/20 mentioned) · Gemini 10% · Perplexity 25%** vs the Jul 6 baseline of 1/19 branded-only. **"Who is Ginny Crisp, PharmD" flipped to MENTIONED (Position 2/3, non-branded, 3 sources) the same day /about went live** — the entity stack (About Person schema + Wikidata + Bing Places + GBP + Shortlister, all built Jul 13-15) corroborating one identity is the likely mechanism; prompt #19 needs no manual re-add, Wix auto-generates it. **Competitors-by-visibility panel: Prescription Benefit Solutions #1 at 15%; Innovative Rx Strategies / SmithRx / Navitus / TransparentRx all 0%** (caveat: within Wix's 20-question basket, not the market; the Jul 6 Google-organic read still shows Innovative leading open search). Top sources being cited instead of rxbs.org on the misses: cms.gov, lawcallahan.com, fortis-rx.com, malegislature.gov. **Still NOT mentioned: the contract-language prompt family** (audit rights in a PBM contract, spread-pricing protective language, who can audit my PBM contract, what is a GER) — exactly the queries the `/contract-language-library` page (build kit ready, next Wix block) answers. Brand perception: Positive; ChatGPT's description accurate ("independent PBM auditor," pharmacist-led, transparency); its flagged gaps = limited public proof points (→ /results, gated on Ginny inputs) and modest public footprint (→ listicle/Reddit layer, ongoing). Next read after the Library page indexes.
  - **Per-engine detail (same run):** **The Ginny entity prompt flipped on ALL THREE engines** (ChatGPT 2/3 · Gemini 2/6 · Perplexity 4/10). **Perplexity already cites PBS on two contract-language prompts:** "What contract language protects a plan sponsor from spread pricing" (Position 6/19) and "What is a GER" (Position 7/15) — the glossary + the Potter spread-pricing piece doing exactly what they were built for; still red there: audit-rights + "who can audit my PBM contract." **Competitor reads differ by engine:** ChatGPT has PBS #1 (everyone else 0%); Gemini has Innovative Rx 15% > PBS 10%; Perplexity has Innovative Rx and SmithRx at 45% vs PBS 25% — Perplexity is both PBS's best engine AND the most contested. **Named source targets to out-answer:** smithrx.com's "how to audit your PBM contract" blog + healthlawalliance.com's PBM-audit glossary own the audit prompts on Perplexity (the /contract-language-library page + audit-rights family aims at both); Gemini leans aon.com / pbmi.com / mercer.com / ifebp.org homepages. **Gemini sentiment is only Neutral** (flags: no public reviews/testimonials → the GBP review seeding + /results; "website simplicity"/no visible educational content on-domain → the answer-page wave; same flag from Perplexity: "no visible blog, news section, or educational content" — the off-domain Substack is invisible to engines reading rxbs.org, which is the whole argument for the on-site canonical twins). Perplexity referral sessions: 0 (citations precede traffic; watch this number as the conversion signal).
- **Jul 15, 2026 (the entity page live):** **`rxbs.org/about` rebuilt and published** (Brett) — origin story, Ginny credential card, bench card, earned-media chips linking her Potter byline piece + the SHRM Honest HR episode, and **Organization + Person JSON-LD with `alumniOf` (UNC Eshelman) + the PharmD credential**: the direct fix for the "Who is Ginny Crisp, PharmD" prompt the Jul 6 baseline rejected for thin content. **To-do: re-add prompt #19 to the Wix AI Visibility bank.** Old "Our Story" / "Meet the Team" pages hidden from nav; deletion + 301s to /about awaiting Ginny's OK.
- **Jul 14, 2026 (Sprint 1 effectively complete + the broker money page live):** **Shortlister vendor profile verified and built** (overview with 2025 proof anchors, 4 product listings, RFI inbox live — the path into their "Top Pharmacy Benefits Consultants" listicle; next move is the 3b listicle-inclusion pitch). **`/for-brokers` live at rxbs.org** (Service + BreadcrumbList JSON-LD, indexing requested) — the second persona's owned page joins the citation surface. Same day: Ginny's Potter spread-pricing piece live on HEALTH CARE un-covered (third-party branded mention + contributor corroboration for the Wikidata items). Sprint-1 remainder unchanged: Crunchbase (awaiting contribution-enabled email) + Manta/Alignable.
- **Jul 13, 2026 (entity + off-site signals went live):** Wikidata items created — **Prescription Benefit Solutions Q140537123** + **Ginny Crisp Q140537997**, cross-linked and referenced (the LLM entity-grounding fix). Same day: Google Business Profile claimed + cleaned (address hidden, service-area US, socials linked, review link captured), **Bing Places listing created manually** (matching NAP + description + LinkedIn/Facebook socials; the direct feed into ChatGPT's retrieval index), Shortlister vendor profile submitted (approval pending), `/glossary` live since Jul 10 with indexing requested. Sprint-1 remainder: Crunchbase + the Manta/Alignable NAP sweep (~20 min, no clocks). Expect movement first on the branded/entity prompts (#19-20), then the definitional set as the glossary indexes. Also of note: first documented organic-search Substack acquisition (Source: Google, Jul 13) and 4 paid subs incl. fund-office + fiduciary-advisor readers.
