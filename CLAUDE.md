...
- **Primary KPIs — goal weighting (added Jul 3, 2026, from the 6-month strategy audit, Ginny-approved):** the content system is judged on two primary KPIs: **(1) qualified conversations** (SQL/PARTNER leads that become held calls/meetings; target 4-6/month by October 2026) and **(2) new non-Hylant broker relationships** (target 3-5 active by December 31, 2026; ~95% of client flow currently comes through Hylant, which stays the anchor; this is diversification alongside it, never away from it). Secondary/leading indicators: toolkit leads/month, LinkedIn followers. Monitor-only: newsletter subs (mechanically ~0.6x of follower growth via LinkedIn's auto-prompt, not separately optimizable), Substack free subs, X followers, AI citations (quarterly). Same-audit standing decisions: **paid-Substack work is ON HOLD ~12 months** (2 paid subs / ~$160 in 5 months; free Substack = citation surface + lead capture; revisit if free subs pass ~500); **X cadence stays at full 3+/day** (Ginny's call, Jul 3, 2026); **Q4 content builds through the normal `/build-pipeline` loop** as the weeks approach, not front-loaded. Conversion infrastructure of record: `broker_partner_track.md` (broker diversification engine), `email_gated_toolkit/emails/06`-`08` (post-sequence nurture: monthly briefing, September renewal reactivation, repeat-download follow-up), `email_gated_toolkit/request_a_call_form_spec.md` (replaces mailto CTAs; NO calendar-exposing scheduler, the admin schedules by email per Ginny's preference), `email_gated_toolkit/sales_hour_runbook.md` (weekly protected sales hour). Full audit: `strategy_audit_2026_07.md`.
- **Maintenance-model escalation rule (added Jul 6, 2026, required behavior for every future session):** the repo's procedures are built to be executable by any model, but four classes of work must STOP and FLAG (OPEN_ITEMS + the approvals issue) rather than proceed: (1) **novel strategy** — anything not already covered by `MASTER_GAMEPLAN.md`, the growth plan, or the blueprint gets proposed, never self-approved; (2) **legal-adjacent language** — any edit to the Standards' independence criteria, the findings-guarantee terms, Index/Fine Print Awards naming rules, or the Fiduciary File claim language (counsel-gated, always); (3) **rule conflicts** — when two rules of record contradict, flag with both citations instead of picking one; (4) **published-surface deletions/renames** — URLs, live Substack posts, live funnel components. The high-stakes publishing class (Index, Awards, Standards, guarantee, ads, case studies, Potter pieces, comment letters) always passes `/red-team` before publish and `/verify-fact` on its external claims at use-week. When in doubt about whether something is in scope for autonomous action, it isn't.
...
- **Reveal-slot cadence (added Jul 3, 2026, Ginny-approved — required behavior):** the shocking-structural-reveal is the #1 organic conversion engine (proven twice organically; see Performance Data), so it runs on a cadence, not one slot. **Thursday = the anchor reveal every week** (full Thursday Excellence). **Non-Library Tuesdays = a standing reveal slot** (visual form: Bypass/Funnel or conceptual infographic) — this supersedes the 6-week Tuesday pillar-cycle fill on non-Library Tuesdays; **Library Tuesdays stay the Contract Comparison carousel** (different job). **Wednesday = FLEX reveal** — a POV-text Wednesday may be a reveal when a distinct fact is available and the week isn't saturated; video Wednesdays stay video. Guardrails: every reveal in a week is a DIFFERENT hidden mechanism (triple-distinct holds; none collides with Monday); keep monthly format variety so the reveal doesn't become wallpaper; the binding constraint is fact supply (feed `shocking_fact_bank.md` via `/system-audit` + `/quarterly-research`); if the 2nd weekly reveal reliably lands well below the 1st, that's fatigue → cap tighter. Full operational rule + the 7-element formula + measurement gate: `week_build_spec.md` §2 + `shocking_fact_bank.md` → "Why it works."
...
- **Comment/reply voice rule — net new value or silence (added Jul 7, 2026, required behavior):** every comment and reply from Ginny (LinkedIn, X, Substack) must add something not already in the post or the comment it answers: one of six moves (go deeper, field example, distinction, genuine question, concede-and-refine, human beat). The restate/reframe tic (validate-then-echo openers, "Great question!" + reframe, thanks + self-restatement) is banned; **a like is a complete response** to praise. Run the delete test on every drafted reply: cover the first sentence, and if nothing new remains, don't post it. Any session drafting reply/comment/engagement-patrol copy reads `ginny_voice_comments_replies.md` first. Brett's first-hour comments follow the same bar (`brett_linkedin_positioning.md`).
- **Voice-lock system (added Jul 7, 2026, required behavior):** Ginny's voice is codified in two files any model must read BEFORE drafting anything in her voice: `ginny_voice_canon.md` (annotated examples of her best measured copy + contrast pairs + the delta log) and `ginny_voice_fingerprint.md` (the mechanics: syntax/rhythm, diction, signature constructions, evidence handling, channel deltas, the never list). The gate is `/voice-check` (before → after line edits, wired into `/critique` item 10 and `/critique-fix`). The feedback loop is `/log-voice-delta`: whenever Ginny meaningfully edits drafted copy before publishing, paste the before/after and run it — deltas log to the canon, and patterns recurring 2-3+ times promote into the fingerprint. `/system-audit` §6 watches for drift. Do NOT write adjective-style brand-voice essays; the canon + fingerprint + gate IS the voice documentation.
...
- **Shocking-structural-reveal is the #1 ORGANIC conversion engine (added Jul 3, 2026, proven twice organically).** A genuinely-unknown hidden-structure fact, built to the 7-element formula, reaches non-followers with no amplifier: the offshore-aggregator reveal hit 17,816 and the W26 manufacturer-payments reveal hit **5,502 / 86% out-of-network / +11 followers / 18 saves, fully organic**, while holding the plan-sponsor demo (Senior / 10,001+ / H&HC). It **out-converts Cuban amplification per impression** (~20 vs ~7.6 followers/10K, 9x saves) and, unlike Cuban, is repeatable and in our control. It is now a **cadence, not a single slot**: Thursday anchor every week + non-Library Tuesdays (standing) + Wednesday flex — the reveal-slot cadence in `week_build_spec.md` §2, built to the formula in `shocking_fact_bank.md` → "Why it works." **Cuban refinement:** his *repost* breaks out dollar-comparison / decoder / named-adversary / urgency-POV shapes (a renewal POV went 411 → 7,890), but NOT announcement/origin shapes (Potter capped 3,524), and his borrowed reach converts weakly (2 saves vs the reveal's 18). The lever is the reveal, not the amplifier. **Latest channel totals (Jul 3, 2026):** 2,183 LinkedIn followers (+57 wk), 1,126 newsletter subs (+40), 115 Substack (2 paid, +7), 157 X (+10).
- **Decoder shape is the most consistent conversion engine.** Three of top nine posts are decoder plays across two formats (messy infographic, text post). Floor: 20-25K impressions. Conversion rate: 9-12 followers per 10K, roughly 2-4x broad-topic content. Decoder content is the primary path from LinkedIn feed to newsletter subscriber, not just engagement. (Note the June 2026 split: the decoder shape divides into the high-ceiling **shocking hidden-structure reveal** above and the lower-ceiling **named-actor vocabulary decoder** ~4-5K summer; when this section says "decoder," the reveal is the conversion workhorse.)
...
**Post-sequence layer (added Jul 3, 2026, from the strategy audit — the sequence no longer ends at Day 14):** `email_gated_toolkit/emails/06_monthly_briefing_template.md` (monthly "PBS Briefing" to captured leads, first Tuesday 9:00 AM EST; **leads-only list, NEVER import the Substack subscriber list into Wix** — consent/deliverability + already-nurtured + intent mismatch; auto-drafted on the 25th by `monthly-briefing-draft.yml` to `email_gated_toolkit/briefings/`), `07_renewal_season_reactivation.md` (one-shot to the full lead list, **Tue Sep 8, 2026**, timed to the 90-day Jan-1 notice windows; supersedes the September briefing per the fortnight-spacing rule), and `08_repeat_download_followup.md` (behavior-triggered personal note from Ginny at 3+ downloads or SQL/PARTNER 2nd download; bulk downloaders 10+ skip it and go straight to the Broker Partner track P1). The `request_a_call_form_spec.md` replaces every mailto: CTA once built (no calendar-exposing scheduler; the admin schedules by email, per Ginny's preference; `?topic=` pre-set links; CallRequests CMS + instant alert + Sheet row). Mailto swaps happen only AFTER the page renders live.
...
```
/home/user/pbs-content/
├── CLAUDE.md                                 # This file - session context
├── PBS_CONTENT_PROJECT_SUMMARY.md           # Full project summary
├── strategy_audit_2026_07.md                # 6-month strategy audit (Jul 3, 2026): diagnosis + 30/90-day plan
├── kpi_scoreboard.md                        # The two primary KPIs, logged monthly via /log-kpis
├── broker_partner_track.md                  # Broker diversification engine (Hylant-concentration fix)
├── target_account_list.md                   # Named-account layer: ~50 employer / ~20 broker targets + weekly motion
├── quarterly_briefing_event.md              # Quarterly live briefing playbook (first: Sep 2026)
├── engagement_closeout_kit.md               # Proof engine: win-story capture + testimonial/referral asks
├── case_studies/                            # Win stories (internal until permissioned)
├── pbs_q1_2026_content_calendar.md          # Q1 calendar + performance data
├── pbs_q1_2026_post_copy.md                 # Detailed LinkedIn post copy
├── pbs_q2_2026_content_calendar.md          # Q2 calendar + seasonal framework (Q3 calendar also present)
...
├── email_gated_toolkit/                     # Lead funnel: Wix/Velo/Zapier specs, emails 01-08, briefings/, sales_hour_runbook.md, request_a_call_form_spec.md
└── newsletters/
    ├── templates/
...
- **THE STEERING DOCUMENT — start here when Brett asks "what's next" (added Jul 6, 2026):** `MASTER_GAMEPLAN.md` is the single prioritized order of ALL outstanding operations (P0→P5, owners tagged, plus the coordinated rollout calendar for every asset). Its companions: `DECISIONS_FOR_GINNY.md` (every open decision with a recommendation), `growth_plan_2026_2027.md` (the business plan), `category_leadership_blueprint.md` (the 5-mechanic horizon). **Publish-ready assets awaiting their rollout slot (page copy lives between ▼▲ markers; never link a page before it renders live):** `standards_independent_pbm_review.md` (/standards page + PDF), `renewal_second_opinion_kit.md` (landing page + the 90-Day Check ritual), `fiduciary_file_onepager.md` (page sections + 1-page PDF; counsel-gated), `pricing_architecture_memo.md` (architecture options + "How we charge" page copy), `sales_conversation_playbook.md` (INTERNAL, Ginny's calls, never publishes), `findings_guarantee_design.md` (gated: analyst data pull + counsel + Ginny), `transparency_index_methodology.md` (analysts execute Nov-Dec 2026; publishes mid-Jan 2027 with the Fine Print Awards), `contract_decoder_spec.md` (Q1 2027 build), `benchmark_coop_spec.md` (Q4 soft-launch), `dollar_fable_creative_brief.md` (designer-ready; mails early Sep), `audio_layer_and_certification.md` (audio starts anytime; certification build-triggered). Any session asked to advance one of these: read the doc, respect its stated gates, and follow the rollout calendar's ordering + cascade rules in `MASTER_GAMEPLAN.md`.
- **Full project context:** PBS_CONTENT_PROJECT_SUMMARY.md
- **6-month strategy audit (Jul 3, 2026 — the goal-weighting + conversion-vs-content diagnosis of record):** strategy_audit_2026_07.md
- **Conversion & sales infrastructure (added Jul 3, 2026, all from the audit + world-class-gap review):** `kpi_scoreboard.md` (the two primary KPIs, logged monthly via `/log-kpis`; blank beats guessed) · `broker_partner_track.md` (the Hylant-diversification engine: enablement kit incl. `templates/documents/broker_partner_one_pager.pdf`, P1-P3 templates, relationship tracker with COLLABORATING+ as the KPI) · `target_account_list.md` (named-account layer: ~41 employer + 22 broker-firm researched CANDIDATE rows pending Ginny's prune pass, incl. 2 CONFIRMED self-funded municipalities; SC school districts are mandatory-PEBA → never targets; worked as sales-hour step 3b, 3 value-touches/week, no cold pitching, no contact scraping) · `quarterly_briefing_event.md` (quarterly live "What We're Seeing in PBM Contracts"; first event recommended Wed Sep 16, 2026 12:00 PM ET; Zoom registration = lead capture) · `engagement_closeout_kit.md` + `case_studies/` (the proof engine: win-story capture at findings, testimonial/referral asks never in the same email, publish nothing without permission, broker-hero framing always) · `email_gated_toolkit/sales_hour_runbook.md` (the weekly protected hour: hot leads → threads → account touches → broker move → stale sweep → log)
...
- **Wendell Potter contributorship (status as of Jul 6, 2026):** Opening piece PUBLISHED Jun 17; Piece 1 (spread pricing) drafted, awaiting Ginny review + submission-week fact re-verify; **Pieces 2 THROUGH 12 are ALL drafted to the corrected house standard (Jul 6, 2026)** — the full remaining roster (`wendell_potter_piece_02..12_*_draft.md`, each with arc machinery + verify-flags in the header; Piece 4's May-era drafts are SUPERSEDED by `piece_04_copay_accumulators_draft.md`). Drafting procedure for future pieces/recalibrations: `/draft-potter-piece`. Submission-week gates: `/verify-fact` on each header's flag list + `/red-team` (Piece 11's broker-read attack is mandatory). Piece 6's 'since the report' section carries an explicit reconcile-at-submission flag (the federal reform landscape moved in 2026). **The editorial standard of record is `wendell_potter_editorial_style_analysis.md` (Rules 1-10, from Ginny + Wendell's final copy): teach generously in the body, gloss roles/acronyms/parents in stride, NO engineered pull-quotes or parallel-punch runs, no client homework, opinion stated openly then a flat forward-pointing close.** The piece OUTLINES predate this standard: keep their beats, discard their rhetorical instructions (pull-quote engineering etc.). Piece 4's old drafts predate the standard too; recalibrate before use. Every piece carries the Series Narrative Arc machinery (callback / map anchor / seed / handoff, `wendell_potter_piece_outlines.md`). Cascade weeks stay gated on Wendell's cadence confirmation (`wendell_potter_cascade_alignment.md`). Post-series: `wendell_potter_post_series_plan.md` (anthology, the January Index as the annual sequel slot, quarterly-column conversion, syndication ladder, testimony readiness).
...
- **Automation system (scheduled workflows + quality gates):** `.github/workflows/README.md` (21 workflows: weekly pipeline/roundup/critique-auto-fix/monday-brief/substack-url-autofill/sales-hour-brief, monthly system-audit/AEO-page/Substack-backfill/citation-reminder/platform-research/AEO-research/search-console/video-research/ads-research/briefing-draft/critique-sweep/kpi-prompt, weekly metrics-reminder, quarterly research, push-time brand-lint; **Tier 1 conversion-side loops added Jul 3, 2026:** `weekly-substack-urls` (RSS-verified auto-fill of `[SUBSTACK ... URL]` placeholders in the current week file + a publish watchdog that warns the rolling issue when the day's expected post isn't in the feed by 10:30 AM ET), `monthly-briefing-draft` (`/draft-monthly-briefing` drafts Email 06 to `email_gated_toolkit/briefings/` on the 25th; leads-only list, never the Substack list; auto-skips a month when Email 07 is within 2 weeks), `monthly-critique-sweep` (2nd Saturday, `/critique-fix` on the two weeks beyond next-to-ship), `monthly-kpi-prompt` + `/log-kpis` + `kpi_scoreboard.md` (the revenue-KPI logging loop for the two primary KPIs), `weekly-sales-hour-brief` (Friday queue for `sales_hour_runbook.md` with a drafted broker touch note); the weekly critique now runs Wed night via `/critique-fix` and AUTO-APPLIES resolvable fixes to the week file, leaving only unresolvable items (external URLs, human data) flagged for Thursday scheduling; run-notifications post to one rolling GitHub issue that @mentions + is assigned to `@crispjb`; the notify steps use `${{ secrets.NOTIFY_PAT || secrets.GITHUB_TOKEN }}` so a real-user PAT makes runs actually email — bot/`GITHUB_TOKEN`-authored comments never notify, which is why subscribing alone produced no email; add the `NOTIFY_PAT` secret to turn email on). **Two issue surfaces (added Jul 1, 2026):** run notifications go to **"🔔 PBS automation log"** (via `notify_issue.sh`; every run that committed appends a "Review this run's changes" commit link, or a ⚠ warning if its push failed), and judgment-call approvals go to a SEPARATE standing **"✅ PBS — Approvals needed"** issue (via `request_approval.sh`) as **tap-to-approve GitHub checkboxes** — Ginny checks the ones she approves from mobile, then a Claude session builds each checked item via a review-linked PR and ticks it off. The audit/quarterly/ads/video commands write their "needs Ginny" items as `- [ ]` lines to gitignored `APPROVALS_PENDING.md`, which a "Surface approvals" workflow step posts. Checking a box records approval only; it does not auto-build. Every committing workflow rebases-then-retries its push so concurrent runs (e.g. monthly+quarterly on the 1st) can't reject each other and lose work. The **daily first-comments reminder runs as a scheduled Claude Code web session** (Triggers, configured in the web app, outside the repo) invoking `/first-comments-today` and delivering to the Claude app, NOT a GitHub workflow. Brand/schema linter: `tools/brand_lint.py` (also a Stop hook). **Cron convention (added Jul 5, 2026, after the KPI prompt fired 3 days straight):** GitHub cron treats day-of-month + day-of-week as **OR**, never AND — so "first Monday"/"2nd Saturday" schedules must use a day-range-only cron (`1-7`/`8-14`) plus an in-workflow weekday guard step (manual dispatch always proceeds); never encode both fields in one cron line. Content loops draft to `main` for review; nothing auto-publishes.
...
*Last updated: July 7, 2026 (voice-lock system: canon + fingerprint + /voice-check gate + delta loop)*
