...
- **Voice fingerprint violations (critique item 10):** banned phrases/intensifiers/corporate verbs, exclamation marks in professional copy, metronomic rhythm, an adversarial claim missing its actor/mechanism/who-pays part, emphasis-dressed escalation, validate-then-echo openers in drafted reply copy → apply the `/voice-check` before → after line edits directly (line edits are resolvable; a full-post voice REDRAFT verdict is a judgment call → flag it).
