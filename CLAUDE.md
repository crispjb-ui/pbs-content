...
(a) **Feed the Library continuously.** Whenever content-building surfaces a contract provision that is not already in the main sections of `substack_contract_language_library.md`, append it to the current-quarter Update Queue in that file *before* the week's work is committed. This applies when drafting a new Library NN carousel, a field note on contract language, a "What I'd Ask" scenario, or any week file that redlines a clause. The queue entry captures: provision name, destination section, weak "what you might see" language, plain-English interpretation, protective "what to ask for" language, source week.

(b) **Alert at quarter boundaries.** At the first session of any new quarter (weeks containing Jan 1 / Apr 1 / Jul 1 / Oct 1), proactively open `substack_contract_language_library.md`, review the outgoing quarter's Update Queue, and surface a "ready to push to Substack" brief for Ginny. The brief lists each queued provision in library format (paste-ready), flags any existing provision that needs expansion, and identifies the LinkedIn and Substack Notes announcement copy. Ginny updates the live Substack post from the brief. **Do not wait to be asked** — the quarter-boundary check is a standing task that runs proactively.
