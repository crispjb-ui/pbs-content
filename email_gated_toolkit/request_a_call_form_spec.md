...
- [x] Ginny: page **fully built and live** at `rxbs.org/request-a-call` (Jul 8, 2026) — form + surrounding copy (eyebrow/H1/subhead/reassurance/secondary-email lines) all in. Notification chain wired via Zapier (Catch Hook → team notification email w/ lead tier/downloads/source → requester confirmation email), **both tested and confirmed**. Topic dropdown "Claims review" → **"Pharmacy Benefits Review"** (Velo `TOPIC_MAP` synced in repo; `?topic=` key = `pharmacy-benefits-review`). **JSON-LD `ContactPage` schema live + validated** (validator.schema.org, 0 errors/0 warnings, Jul 8). _One item left on this page:_ the "Privacy" link stays link-suppressed until the `/privacy` page publishes (counsel-gated).
