...
- [ ] **6-beat architecture (text posts only — Wed POV, Fri tease, newsletter publish copy, longer X; NOT visual-first carousels/infographics/video):** each text post runs Hook → Promise (substantive payoff line, not influencer "in the next 90 seconds" voice) → Positioning (proprietary anchor, *varied/fresh* — rotate 2025 metrics, never a repeated boilerplate footer) → Core Message (formatted, ~1,400-1,800 chars) → Conclusion (one crystallized takeaway line) → Call to Engage (question or bold statement, never a commercial CTA in body; closed Q for comments, open Q for reach/DMs). Where a beat conflicts with WORKING/WEAK measured data, the data wins. (Full rationale: CLAUDE.md "6-beat post architecture" rule; enforced in `/critique` item 9.)
