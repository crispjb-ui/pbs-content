# Week 34 Newsletter Bundle: Specialty Drug Management
## August 24-28, 2026
...
## Specialty Drug Management: The Category That Defines Your Plan's Cost Trajectory

**Subtitle:** An eight-factor assessment scored across three dimensions that reveals where your specialty strategy has gaps and what to fix first

**Publication:** Benefit Blind Spots
**Publish:** Monday, August 24, 2026 at 7:30 AM EST
...
- **Below 60:** Significant specialty management gaps. This should be the top priority in your renewal negotiation. Focus on the levers with the highest improvement opportunity scores first.
- **60-90:** Moderate management. Some levers are well-managed, others have gaps. Target the bottom three levers for improvement.
- **Above 90:** Advanced specialty management. Focus on maintaining what works and watching for emerging gaps as new specialty drugs launch.
...
*For the full protective contract language on specialty routing and prior authorization, see our free PBM Contract Language Library: https://www.rxbs.org/contract-language-library. Updated quarterly from patterns we see in client contract reviews.*
...
**Tags:** specialty pharmacy, drug management, biosimilars, site of care, self-funded employers, cost containment
...
## The Pharmacy Benefits Briefing - Issue #29
### Specialty Management Briefing

**Publish:** Monday, August 24, 2026 at 7:45 AM EST
**Length:** ~400 words
...
Specialty pharmacy now represents the majority of total pharmacy cost for many of our clients. The shift happened gradually. Each year, specialty grew as a percentage. Each year, employers focused on retail and mail while specialty ran on the PBM's default settings.

Here is the problem: specialty gets the least independent oversight despite driving the most cost. The PBM manages specialty routing (often through a pharmacy they own). The PBM manages specialty formulary (influenced by rebate economics). The PBM reports specialty savings (using their own methodology). Unless the contract requires otherwise, the employer's cost objective is not the driving force.

Eight levers determine whether specialty cost is controlled or catastrophic: formulary management, site-of-care, step therapy, biosimilar adoption, limited distribution access, copay card programs, clinical pathways, and reauthorization protocols. Each one either contains cost or allows it to grow unchecked.

Most plans we review have strong positions on two or three of these levers and gaps on the rest. The levers with gaps are where the unrealized savings live. And the gap between managed and unmanaged specialty can be substantial over a plan year.

The fix is not one big decision. It is eight specific evaluations, each connected to contract provisions and claims data. Start with the lever that matches your highest specialty spend category.

---

This week's Benefit Blind Spots includes a **specialty drug management assessment**: eight factors scored across three dimensions (strategy strength, contract support, improvement opportunity) with a 120-point scale and priority action list. That assessment is exclusive to the Substack: [SUBSTACK URL]
...
```
Specialty is most of your spend on a fraction of your prescriptions, and the part of your plan with the least independent oversight.
...
The question is whether you have your own strategy built from the same data, for your plan's objectives. If you do not, you are negotiating within their framework by default.
...
**Mon Aug 24**
- **8:00 AM · recipe / J-code billing gap (Tier B)** _(re-concepted Jul 9: the prior maximizer recipe ran the day after W33 Sunday's maximizer Tier A, a consecutive-day concept repeat)_:
```
...
- **5:00 PM · punchy:**
```
The report your PBM sends is the report your PBM survives. Ask for a field that is not in it and watch the tone of the meeting change.
```
...
```
The exclusion letter tells your members which drugs lost coverage. It never says what the surviving drugs paid to stay. Ask for net cost on every drug that kept its spot in a class where something got dropped. That is the list worth reading.
```
  First reply (link): rxbs.org/toolkit/step-therapy-override
- **12:00 PM · news-reaction HOLD** (fire same-hour on breaking PBM news; tag the source). Quiet-day backup:
```
The same infusion costs your plan one price in a hospital outpatient chair and a fraction of that at home or in a clinic. The drug did not change. The address did. Site of care is a pricing decision wearing a geography costume.
```
- **5:00 PM · punchy:**
```
The PBM knows your plan's data better than you do, and it prices that asymmetry into every renewal.
```
...
```
A GER guarantee leaves two levers in the PBM's hands: which claims count and when the measurement period closes. Move enough claims between buckets and a missed guarantee becomes a met one. The rate never changed. The denominator did.
```
  First reply (link): rxbs.org/toolkit/ger-audit
- **12:00 PM · news-reaction HOLD** (fire same-hour on breaking PBM news; tag the source). Quiet-day backup:
```
Your contract quotes discounts off AWP, pays pharmacies off MAC, and runs rebate math off WAC. Three price languages, one translator, and the translator sends you the bill.
```
- **5:00 PM · punchy:**
```
PBM contracts are written once and misread for three years. The misreading is the product.
```
...
```
This year's PBM invoice carries more fee lines than last year's. Same services, new names: clinical support, data access, network management. Unbundling turns one negotiated fee into five unnegotiated ones. Count your lines year over year. That trend is a price increase that never got announced.
```
...
- **5:00 PM · punchy:**
```
You do not need a law degree to read a PBM contract. You need to know which five definitions to read first. The PBM is betting nobody told you.
```
- **7:00 PM · question post [A1]:**
```
Has your plan ever seen the pharmacy-paid amount next to the plan-billed amount for the same generic fill? That gap is the spread. It funds a lot, and it has never once been volunteered on a report.
```
  First reply → Here is how to find it on your own claims: rxbs.org/toolkit/pbm-compensation
- **8:30 PM · one question [B2]:**
```
Put this on the next PBM meeting agenda, verbatim: "Total manufacturer dollars received on our book last year, all payment labels, versus total remitted to us." Then let the silence do its work.
```

**Fri Aug 28**
- **8:00 AM · recipe / Network access fee (Tier B):**
```
Somewhere on your invoice sits a network access fee. The network is mandatory. The pharmacies in it already signed the PBM's terms. Ask what the fee buys that the contract does not already require. There is no good answer, which is the point.
```
...
```
Specialty routing in one experiment: ask your PBM to admit one independent specialty pharmacy to the network at parity terms. The response letter will teach you more about your network than the adequacy report ever did.
```
