...
- [x] ✅ CLOSED Jul 22 — **Toolkit form surfaces:** the global footer's "Privacy Policy" link renders on all 29 toolkit landing pages (footer is master-page-level, dynamic Toolkits (Item) included) — verified in the editor by Brett. Welcome-email side: compliance footer (company + Charleston, SC + `rxbs.org/privacy` + unsubscribe line) baked into emails 01-05 repo bodies same day; Brett pastes into Zapier steps 3/5/7/9/11. Physical-address line joins when the PO Box / virtual address exists (pre-Sep-ads to-do).
