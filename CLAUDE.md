...
- **Automation system (scheduled workflows + quality gates):** `.github/workflows/README.md` (12 workflows: weekly pipeline/roundup/critique/monday-brief, monthly system-audit/AEO-page/Substack-backfill/citation-reminder, weekly metrics-reminder, quarterly research, push-time brand-lint; all run-notifications post to one rolling GitHub issue that @mentions + is assigned to `@crispjb`). Brand/schema linter: `tools/brand_lint.py` (also a Stop hook). Content loops draft to `main` for review; nothing auto-publishes.
