# Partner Outreach — Cristy Gupton / Custom Benefits Solutions

_First lead through the live Wix toolkit funnel scored PARTNER. Source: toolkit download, alert email Jun 9, 2026 (6:06 AM). Role = Broker / Consultant, score 0 → PARTNER tier. Created June 9, 2026._

## Who / context
- **Cristy Gupton** — Founder & President, **Custom Benefits Solutions** (Western North Carolina). 22-year career, 3rd-generation insurance, founded the firm in 2006. Email: cristy@custombenefits.work.
- **Transparency profile, gold standard.** Core focus is "Trust, Transparency, Technology," with full compensation disclosure to clients. **Vetted by both Health Rosetta and Mitigate Partners**, the two national alliances built around transparent, high-performance self-funded plans. This is the textbook "broker as ally," not adversary.
- **Hosts the Healthcare Solutions podcast** (since 2019). Has covered PBM fiduciary risk, prescription drug costs, hospital negotiation, and healthcare transparency.
- **Pharmacy read:** she understands the PBM problem conceptually, but the firm has no independent pharmacy/PBM **contract-and-claims audit** capability. That gap is exactly what PBS fills as a behind-the-broker bench.

## Tier / rule
- Funnel tier = **PARTNER** (Broker / Consultant branch). Per `email_gated_toolkit/role_funnel_plan.md` and the CLAUDE.md "brokers as partners" rule: **never pitch a broker an audit.** Track = referral / partnership. PBS is the independent audit bench she brings in; she stays the relationship owner. Metric = clients referred, not a direct close.

## Objective
Open a peer, founder-to-founder relationship. One soft door: a no-cost first look / quiet pharmacy-audit bench she can put in front of her clients, with the broker as relationship owner. Relationship, not a close.

## Hooks to deploy (all real, all specific)
- Same-hymnal alignment: Health Rosetta + Mitigate vetting, comp disclosure, transparent self-funded since 2006.
- Proprietary anchor: "We review hundreds of PBM contracts a year." (True; the load-bearing credibility line.)
- _Podcast (Healthcare Solutions) deliberately NOT offered: the show is dormant, last episode S03E10 ~Jan 2024, no Season 4. A "come on the show" ask would read as out-of-touch. Hold it in reserve as a warm follow-up only if she revives the podcast._

## Cautions
- **No audit pitch to her.** Behind brokers, never around them. Never imply she has missed anything on pharmacy; frame PBS as the specialist hands, she as the relationship owner.
- **PBM is the adversary, the broker is the ally.** Keep that frame.
- **No fabricated numbers.** Deploy the realized-savings track record only if natural, and labeled "results vary by plan." The draft below omits it deliberately on a first touch.
- Confirm which toolkit she downloaded before sending and name it if known (the alert email did not surface the toolkit field). The draft references "our worksheets" generically so it sends correctly either way.

---

## Email draft (paste-ready)

**Subject line options:**
1. From one transparency shop to another
2. Saw your Custom Benefits download, a note from a fellow transparency nerd
3. Behind the broker, never in front of you

```
Cristy,

Your name came through when you pulled one of our plan-sponsor
worksheets, and I went down the rabbit hole on Custom Benefits
Solutions before writing. A Health Rosetta and Mitigate Partners
advisor who discloses comp and has been doing transparent
self-funded work since 2006 is exactly who we built those tools
for. We are clearly reading from the same hymnal.

Quick on who we are. I am a pharmacist by training, and PBS does
one narrow thing well: we review hundreds of PBM contracts a year
and run independent claims and contract audits on the pharmacy
side. We work behind brokers, never around them. The advisors we
partner with bring us in when a client's PBM contract or claims
file needs a set of hands that lives in the pharmacy weeds all
day, and the broker stays the relationship owner the whole way.

I am not writing to pitch you anything. I am writing because the
firms vetted by Health Rosetta and Mitigate are the ones we most
want to know. One open door: if a client's pharmacy spend or PBM
contract ever needs an independent read, we are a no-cost first
look and a quiet bench you can put your name in front of. The
broker stays the relationship owner the whole way.

If that is ever useful, reply here or grab time at team@rxbs.org.
If not, you have the worksheets and I am glad they are in good
hands.

Ginny

Ginny Crisp, PharmD, BCACP | Chief Executive Officer
Prescription Benefit Solutions
team@rxbs.org | www.rxbs.org
```

### Shorter variant (if a lighter first touch is preferred)

```
Cristy,

You pulled one of our plan-sponsor worksheets, so I looked up
Custom Benefits Solutions before writing. A Health Rosetta and
Mitigate advisor who discloses comp and has run transparent
self-funded plans since 2006 is exactly who those tools are for.
Same hymnal.

I am a pharmacist, and PBS does one narrow thing: we review
hundreds of PBM contracts a year and run independent pharmacy
contract and claims audits. We work behind brokers, never around
them. The advisor stays the relationship owner.

No pitch. Just an open door: if a client's PBM contract or
pharmacy spend ever needs an independent read, we are a no-cost
first look and a quiet bench you can put your name in front of.
The broker stays the relationship owner the whole way.

Reply here or team@rxbs.org if that is ever useful.

Ginny

Ginny Crisp, PharmD, BCACP | Chief Executive Officer
Prescription Benefit Solutions
team@rxbs.org | www.rxbs.org
```

## Next step after sending
- Mark status = "contacted" in the PBS Toolkit Leads sheet (per the alert email).
- Treat as relationship-first; referrals follow. If she ever revives Healthcare Solutions, a guest spot is the natural warm re-engagement.
