...
- **AI-Detection Pass Rule for Long-Form External Pieces (added May 13, 2026):** Every long-form piece submitted to an external publication (Wendell Potter's HEALTH CARE un-covered as the active example; future invited contributor placements as they arise) must pass an external AI-detection screening before submission. The PBS Humanize Check (Apr 19) is the floor that establishes voice; this rule is the ceiling that ensures the piece survives detection tools the receiving publication's editorial workflow may run. **Workflow:** (a) Draft per Wendell-channel voice rules in `wendell_potter_contributorship_strategy.md` (metaphor-led opener, prescriptive ending, named adversaries by company, opinion stated openly with expertise anchor). (b) Run through Originality.ai detection as the primary tool (~$0.01 per 100 words; weighted toward catching GPT-4 and Claude output; used by publishers). Target <10% AI-likelihood. (c) Cross-check on GPTZero as the secondary tool ($9.99/mo Plus tier; different methodology). Both methodologies should agree under 10%. (d) **If either flags >10%, apply the failure-mode revision techniques:** introduce contractions (don't, isn't, can't, I'm, you're, that's) throughout the piece; break perfect triple parallelism patterns ("The reform is X. The reform is Y. The reform is not Z." is a strong AI tell; replace with looser idiomatic shapes); vary burstiness wildly (mix 6-word fragments with 30-word complex sentences); add first-person idiomatic asides ("honestly," "in my experience," "I'll be straight," "I get the impulse"); replace formal connectives (Additionally, Furthermore, Moreover) with idiomatic register; reduce staccato pairs ("X. Y.") to maximum one per piece, used as a load-bearing closing anchor; add parenthetical asides where they fit naturally. (e) Re-run detection after each revision pass until under threshold on both tools. (f) Document final detection scores in the piece-specific draft file footer for record. (g) **PBS LinkedIn-channel winning rhetorical shapes** (the "Same X. Same Y. Same Z." triple parallelism, staccato decoder fragments, dollar-comparison stat-shock openers) score AI-likely on detection tools by design — they are engineered for algorithm engagement, not for detection-tool passage. Use them sparingly in Wendell-channel pieces (maximum one such pattern per ~1,000-word piece, used as a closing rhetorical anchor where the AI-pattern penalty is offset by the rhetorical payoff). Do not stack them. **Channel scope:** This rule applies to Wendell-channel pieces and future external long-form invited placements only. PBS-channel LinkedIn posts, carousels, and image-prompt visual copy are exempt (different optimization target, detection tools struggle with short text anyway). PBS-channel Substack deep dives, Field Notes, and Wednesday roundups are NOT currently covered by this rule but may be added if external republication or syndication becomes part of the strategy. **Companion files:** `wendell_potter_publish_checklist.md` operationalizes this rule into a per-piece checklist.
