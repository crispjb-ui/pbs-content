...
**⚠ RED-TEAM FLAG 1 — RESOLVED PATH CHANGED (Jul 21, 2026, Brett decision):** counsel review waived; the gate is now **Ginny's signed attestation walk** (`standards_independence_attestation.md` §1) confirming the criteria + "We accept no PBM revenue" are true, complete, and durable. Page publishes only after that signature.
**⚠ RED-TEAM FLAG 2 — RESOLVED (Jul 21, 2026):** the attestation template exists (`standards_independence_attestation.md` §2, the client-facing Part 3.4 deliverable); render to the toolkit PDF system once §1 is signed.
...
## ▲ PUBLISH-READY COPY ENDS ▲

---

**Production notes (internal):** brokers appear only implicitly and only in criterion language about ownership, per the broker-hero rule; the "questions to ask" blockquote follows the Substack blockquote convention and doubles as the page's FAQPage-adjacent snippet for AEO; the annual revision pairs with the January Index. PDF render: standard toolkit pipeline once approved. First distribution: the /standards page, a Substack Field Note announcing it, the September campaign citations, and the broker enablement kit.
